...
function ProjectListScreen() {
	...
	function CallAPI() {
		...
		var data = pma.dummyData.getProjectList();
		projectData = data.project_list;
		
		//Titanium.API.warn(projectData);
		//Titanium.API.warn(projectData.project_list);
	...
	for (var i = 0; i < projectData.length; i++) {
		...
		Titanium.API.warn(projectData[i].project_name);
		Titanium.API.warn(projectData[i].project_key);
		Titanium.API.warn(projectData[i].project_lead);
		Titanium.API.warn(projectData[i].member_count);
		Titanium.API.warn(projectData[i].ticket_count);
		Titanium.API.warn("\n/***********************************************************/\n");
		
		//Append rows in table one by one.
		tableview.appendRow(getRow(projectData[i]));

	}

	
	//tableview.appendRow(getRow(projectData));
